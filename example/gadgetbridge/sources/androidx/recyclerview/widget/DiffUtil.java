            ...
            for (int d = 0; d <= dLimit; d++) {
                ...
                while (k2 <= d) {
                    ...
                    if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || iArr[kOffset + backwardK] < iArr2[kOffset + backwardK]) {
                        ...
                        outSnake2.f33x = iArr2[kOffset + backwardK];
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                for (int i2 = snakeSize - 1; i2 >= 0; i2--) {
                    if ((this.mOldItemStatuses[snake.f33x + i2] & 31) == 2) {
                        batchingCallback.onChanged(snake.f33x + i2, 1, this.mCallback.getChangePayload(snake.f33x + i2, snake.f34y + i2));
                    }
                }
                posOld = snake.f33x;
                posNew = snake.f34y;
        ...
        private void dispatchAdditions(List<PostponedUpdate> postponedUpdates, ListUpdateCallback updateCallback, int start, int count, int globalIndex) {
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                if (status == 0) {
                    ...
                    throw new IllegalStateException("unknown flag for pos " + (globalIndex + i) + StringUtils.SPACE + Long.toBinaryString((long) status));
                }
            }
        }
