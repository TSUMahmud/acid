    ...
    public ThreadUtil.BackgroundCallback<T> getBackgroundProxy(final ThreadUtil.BackgroundCallback<T> callback) {
        return new ThreadUtil.BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new Runnable() {
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C04052.this.mQueue.next();
                        if (msg == null) {
                            C04052.this.mBackgroundRunning.set(false);
                            return;
                        }
                        int i = msg.what;
                        if (i == 1) {
                            C04052.this.mQueue.removeMessages(1);
                            callback.refresh(msg.arg1);
                        } else if (i == 2) {
                            C04052.this.mQueue.removeMessages(2);
                            C04052.this.mQueue.removeMessages(3);
