...
public class PieChart extends PieRadarChartBase<PieData> {
    ...
    public void calculateOffsets() {
        ...
        if (this.mData != null) {
            ...
            this.mCircleBox.set((c.f90x - radius) + shift, (c.f91y - radius) + shift, (c.f90x + radius) - shift, (c.f91y + radius) - shift);
    ...
    public float[] getMarkerPosition(Highlight highlight) {
        ...
        double d = (double) r2;
        double cos = Math.cos(Math.toRadians((double) (((this.mAbsoluteAngles[entryIndex] + rotationAngle) - offset) * this.mAnimator.getPhaseY())));
        Double.isNaN(d);
        double d2 = d * cos;
        ...
        float x = (float) (d2 + d3);
        double d4 = (double) r2;
        double sin = Math.sin(Math.toRadians((double) (((this.mAbsoluteAngles[entryIndex] + rotationAngle) - offset) * this.mAnimator.getPhaseY())));
        Double.isNaN(d4);
        double d5 = d4 * sin;
        double d6 = (double) center.f91y;
        Double.isNaN(d6);
        MPPointF.recycleInstance(center);
        return new float[]{x, (float) (d5 + d6)};
    }
    ...
    private void calcAngles() {
        ...
        for (int i3 = 0; i3 < ((PieData) this.mData).getDataSetCount(); i3++) {
            ...
            for (int j = 0; j < set.getEntryCount(); j++) {
                ...
                fArr[cnt] = drawAngle;
                if (cnt == 0) {
                    this.mAbsoluteAngles[cnt] = fArr[cnt];
                } else {
    ...
    public void setCenterTextOffset(float x, float y) {
        this.mCenterTextOffset.f90x = Utils.convertDpToPixel(x);
        this.mCenterTextOffset.f91y = Utils.convertDpToPixel(y);
    }

    public MPPointF getCenterTextOffset() {
        return MPPointF.getInstance(this.mCenterTextOffset.f90x, this.mCenterTextOffset.f91y);
    }
