        ...
        if (labelCount == 0 || range <= Utils.DOUBLE_EPSILON) {
            ...
            if (this.mAxis.isForceLabelsEnabled()) {
                ...
                if (interval == Utils.DOUBLE_EPSILON) {
                    ...
                    double d6 = (double) yMin;
                    Double.isNaN(d6);
                    first = Math.ceil(d6 / interval) * interval;
                }
                ...
                if (interval == Utils.DOUBLE_EPSILON) {
                    n2 = n3;
                    double d7 = rawInterval;
                    last = Utils.DOUBLE_EPSILON;
                } else {
                    n2 = n3;
                    double d8 = rawInterval;
                    double d9 = (double) yMax;
                    Double.isNaN(d9);
                    last = Utils.nextUp(Math.floor(d9 / interval) * interval);
                }
                if (interval != Utils.DOUBLE_EPSILON) {
    ...
    public void renderAxisLabels(Canvas c) {
        if (this.mYAxis.isEnabled() && this.mYAxis.isDrawLabelsEnabled()) {
            ...
            for (int j = from; j < to; j++) {
                ...
                c.drawText(this.mYAxis.getFormattedLabel(j), pOut.f90x + 10.0f, pOut.f91y, this.mAxisLabelPaint);
            }
            ...
            for (int i = 0; i < limitLines.size(); i++) {
                ...
                if (l.isEnabled()) {
                    ...
                    for (int j = 0; j < ((IRadarDataSet) ((RadarData) this.mChart.getData()).getMaxEntryCountSet()).getEntryCount(); j++) {
                        ...
                        if (j == 0) {
                            limitPath.moveTo(pOut.f90x, pOut.f91y);
                        } else {
                            limitPath.lineTo(pOut.f90x, pOut.f91y);
                        }
                    }
                    limitPath.close();
