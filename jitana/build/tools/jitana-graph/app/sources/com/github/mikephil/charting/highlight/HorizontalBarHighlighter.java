...
public class HorizontalBarHighlighter extends BarHighlighter {
    ...
    public Highlight getHighlight(float x, float y) {
        ...
        Highlight high = getHighlightForX((float) pos.f89y, y, x);
        if (high == null) {
            return null;
        }
        IBarDataSet set = (IBarDataSet) barData.getDataSetByIndex(high.getDataSetIndex());
        if (set.isStacked()) {
            return getStackedHighlight(high, set, (float) pos.f89y, (float) pos.f88x);
        }
    ...
    public List<Highlight> buildHighlights(IDataSet set, int dataSetIndex, float xVal, DataSet.Rounding rounding) {
        ...
        for (Entry e : entries) {
            ...
            Highlight highlight2 = new Highlight(e.getX(), e.getY(), (float) pixels.f88x, (float) pixels.f89y, dataSetIndex, set.getAxisDependency());
