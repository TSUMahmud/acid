...
public class BarHighlighter extends ChartHighlighter<BarDataProvider> {
    ...
    public Highlight getHighlight(float x, float y) {
        ...
        if (set.isStacked()) {
            return getStackedHighlight(high, set, (float) pos.f88x, (float) pos.f89y);
        }
    ...
    public Highlight getStackedHighlight(Highlight high, IBarDataSet set, float xVal, float yVal) {
        ...
        if (ranges.length > 0) {
            ...
            MPPointD pixels = ((BarDataProvider) this.mChart).getTransformer(set.getAxisDependency()).getPixelForValues(high.getX(), ranges[stackIndex].f79to);
            Highlight stackedHigh = new Highlight(entry.getX(), entry.getY(), (float) pixels.f88x, (float) pixels.f89y, high.getDataSetIndex(), stackIndex, high.getAxis());
